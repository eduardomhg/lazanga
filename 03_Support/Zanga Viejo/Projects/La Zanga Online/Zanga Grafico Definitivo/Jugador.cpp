...
{
	Mazo^ mazoNoTriunfos = gcnew Mazo();
	for (int i = 0; i < input->getNumCartas(); i++)
	{
		if (!(input->getCarta(i)->getPalo() == mesa->getTriunfo() ||
			input->getCarta(i)->numeroJuego(bastos, bastos) == 39 ||
...
{
	Mazo^ ordenado = gcnew Mazo(*input);
		...
		if (!mesa->getAlguienSolo()) // No se va nadie solo
		{
			...
			case 0: // Soy mano
				if (numeroBaza < 6) // Primeras 5 bazas
				{
					...
					{					
						if (valorCartaNoTriunfoOrden(
							mazoNoTriunfos->getCarta(mazoNoTriunfos->getNumCartas()-1)) >= 7)
							// Si tengo algun Rey, Caballo o Sota de no triunfos
						{
							if (mazoNoTriunfos->getNumCartas() > 1 && valorCartaNoTriunfoOrden(
								mazoNoTriunfos->getCarta(mazoNoTriunfos->getNumCartas()-1)) == 
								valorCartaNoTriunfoOrden(
								mazoNoTriunfos->getCarta(mazoNoTriunfos->getNumCartas()-2)))
								// Si tengo dos o más Reyes, Caballos o Sotas
							{
								if (mazoNoTriunfos->getNumCartas() > 2 && valorCartaNoTriunfoOrden(
									mazoNoTriunfos->getCarta(mazoNoTriunfos->getNumCartas()-2)) == 
									valorCartaNoTriunfoOrden(
					...
					// Si arrastra
				{
					...
					else // Si no tengo para servir o solo tengo Espadilla, Malilla o Bastillo
					{
						...
						else // Tengo alguna pieza de estuche
						{
							if (numeroBaza < 6) // Primeras 5 bazas
							{					
								if (valorCartaTriunfoOrden(mesa->getCarta((numero+3)%4)) > 4)
					...
					else // Si no tengo para servir
					{
						if (numeroBaza < 6) // Primeras 5 bazas
						{
							if (mazoTriunfos->getNumCartas() > 0 && 
								valorCartaTriunfoOrden(mazoTriunfos->getCarta(0)) < 6)
								// Si tengo triunfos menores que un Caballo
							{
								*cartaElegida = *(mazoTriunfos->getCarta(0));
							}
				...
				// Si arrastra mi compañero
				{
					...
					// Si tengo para servir
					{
						if (mesa->getCarta((numero+2)%4)->numeroJuego(
							...
							mesa->getCarta((numero+3)%4)->numeroJuego(
							mesa->getCarta((numero+2)%4)->getPalo(), mesa->getTriunfo()) &&
							valorCartaTriunfoOrden(mesa->getCarta((numero+2)%4)) > 4)
				...
				else // Si no arrastra mi compañero
				{
					...
					else // Si no tengo para servir
					{
						if (mesa->getCarta((numero+2)%4)->numeroJuego(
							...
							mesa->getCarta((numero+3)%4)->numeroJuego(
							mesa->getCarta((numero+2)%4)->getPalo(), mesa->getTriunfo()) &&
							valorCartaNoTriunfoOrden(mesa->getCarta((numero+2)%4)) >= 8)
...
{	
	...
	case 3:
		...
		for (int i = 0; i < 4; i++)
		{
			for (int j = i+1; j < 4; j++)
			{
				if (coeficientes[j] > coeficientes[i])
				{
					temp = coeficientes[i];
		...
		coeficienteMedia = (coeficientes[0] + coeficientes[1] + coeficientes[2])/3;
		...
		{
			if (coeficienteMedia > umbralContra)
				return paso;
			else
				return juego;
		}
...
frasesHablarZanga Jugador::hablarZanga(Mesa^ mesa, frasesHablarZanga usuarioFraseZanga) 
{
	...
	int umbralSolo = 25;
	Mazo^ cartasOriginales = gcnew Mazo(*(mesa->getZanga()));
...
Jugador::Jugador()
{
	this->mazo  = gcnew Mazo();
}

Jugador::Jugador(const Jugador% jugador)
{
	this->mazo  = gcnew Mazo();
	this->nombre = System::String::Copy(jugador.nombre);
	this->numero = jugador.numero;
	*(this->mazo)  = *(jugador.mazo);
	this->algoritmoIA = jugador.algoritmoIA;
	this->voz = jugador.voz;
	this->numCliente = jugador.numCliente;
}
...
Jugador::Jugador(System::String^ nombre, int numero, int algoritmoIA, int voz, int numCliente)
{
	this->nombre = System::String::Copy(nombre);
	this->numero = numero;
	this->mazo  = gcnew Mazo();
...
System::String^ Jugador::getNombre()
{
	return this->nombre;
}
...
void Jugador::ordenarMazo(palos triunfo)
{
	this->mazo->ordenar(triunfo);
}

int Jugador::getAlgoritmoIA()
{
	return this->algoritmoIA;
}

int Jugador::getNumCliente()
{
