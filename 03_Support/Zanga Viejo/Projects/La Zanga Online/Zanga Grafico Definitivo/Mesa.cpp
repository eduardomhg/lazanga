...
Mesa::Mesa(const Mesa% mesa)
{
	...
	for (int i = 0; i < 4; i++)
	{
		cartas[i] = gcnew Carta(*(mesa.cartas[i]));
	}
	for (int i = 0; i < 8; i++)
	{
		bazas1[i] = gcnew Baza(*(mesa.bazas1[i]));
		bazas2[i] = gcnew Baza(*(mesa.bazas2[i]));
	}
	numCartas = mesa.numCartas;
	numPiedrasCentro = mesa.numPiedrasCentro;
	numPiedras1 = mesa.numPiedras1;
	numPiedras2 = mesa.numPiedras2;
	jugadorMano = mesa.jugadorMano;
	jugadorManoPrimero = mesa.jugadorManoPrimero;
	jugadorTriunfo = mesa.jugadorTriunfo;
	numBazas1 = mesa.numBazas1;
	numBazas2 = mesa.numBazas2;
	triunfo = mesa.triunfo;
	alguienSolo = mesa.alguienSolo;
	jugadorSolo = mesa.jugadorSolo;
	piedrasSolo = mesa.piedrasSolo;
	alguienCinco = mesa.alguienCinco;
	jugadorCinco = mesa.jugadorCinco;
	piedrasCinco = mesa.piedrasCinco;
	cumplioCinco = mesa.cumplioCinco;
	alguienTodas = mesa.alguienTodas;
	jugadorTodas = mesa.jugadorTodas;
	piedrasTodas = mesa.piedrasTodas;
	cumplioTodas = mesa.cumplioTodas;
	alguienEstuche = mesa.alguienEstuche;
	parejaEstuche = mesa.parejaEstuche;
	piedrasEstuche = mesa.piedrasEstuche;
	piedrasCambioZanga = mesa.piedrasCambioZanga;
	numPiedrasVirtuales1 = mesa.numPiedrasVirtuales1;
	numPiedrasVirtuales2 = mesa.numPiedrasVirtuales2;
}

Mesa% Mesa::operator =(const Mesa% mesa)
{
	cartas = gcnew array<Carta^>(4);
	bazas1 = gcnew array<Baza^>(8);
	bazas2 = gcnew array<Baza^>(8);
	for (int i = 0; i < 4; i++)
	{
		*(cartas[i]) = *(mesa.cartas[i]);
	}
	for (int i = 0; i < 8; i++)
	{
		*(bazas1[i]) = *(mesa.bazas1[i]);
		*(bazas2[i]) = *(mesa.bazas2[i]);
	}
	numCartas = mesa.numCartas;
	numPiedrasCentro = mesa.numPiedrasCentro;
	numPiedras1 = mesa.numPiedras1;
...
void Mesa::barajar(Baraja^ baraja, int semilla)
{
	baraja->barajar(semilla);
}

void Mesa::repartir(Baraja^ baraja, array<Jugador^>^ jugadores)
{
	numCartas = 0;
	numBazas1 = 0;
	numBazas2 = 0;
	for (int i = 0; i < 4; i++)
	{
		jugadores[i]->setMazo(gcnew Mazo(baraja, (4+i-jugadorManoPrimero)%4));
	}		
	this->zanga = gcnew Mazo(baraja, 4);		
}
...
void Mesa::organizarBaza()
{
	...
	case 0:
		{
			this->bazas1[numBazas1] = gcnew Baza(cartas[0], cartas[1], cartas[2], cartas[3]);
			this->numBazas1++;
			this->jugadorMano = 0;	
			break;
		}
	case 1:
		{
			this->bazas2[numBazas2] = gcnew Baza(cartas[0], cartas[1], cartas[2], cartas[3]);
			this->numBazas2++;
			this->jugadorMano = 1;	
			break;
		}
...
void Mesa::organizarPiedras()
{
	...
	if (alguienCinco)
	{
		if (jugadorCinco == 0 || jugadorCinco == 2)
		{
			if (cumplioCinco)
			{
				numPiedras1 += minimo(piedrasCinco, numPiedras2);
				numPiedras2 -= minimo(piedrasCinco, numPiedras2);
			}
			else
			{
				numPiedras2 += minimo(piedrasCinco, numPiedras1);
				numPiedras1 -= minimo(piedrasCinco, numPiedras1);
			}
		}
		else
		{
	...
	if (alguienTodas)
	{
		if (jugadorTodas == 0 || jugadorTodas == 2)
		{
			if (cumplioTodas)
			{
				numPiedras1 += minimo(piedrasTodas, numPiedras2);
				numPiedras2 -= minimo(piedrasTodas, numPiedras2);
			}
			else
			{
				numPiedras2 += minimo(piedrasTodas, numPiedras1);
				numPiedras1 -= minimo(piedrasTodas, numPiedras1);
			}
		}
		else
		{
	...
	if (numBazas1 > numBazas2)
	{
		if (alguienEstuche)
		{
			numPiedras1 += minimo(piedrasEstuche, numPiedras2);
			numPiedras2 -= minimo(piedrasEstuche, numPiedras2);
		}
		if (jugadorTriunfo == 0 || jugadorTriunfo == 2)
		{
			...
			if (alguienSolo)
			{
			numPiedras1 += minimo(piedrasSolo, numPiedras2);
			numPiedras2 -= minimo(piedrasSolo, numPiedras2);
			}				
		}
		else
		{
	...
	else if (numBazas2 > numBazas1)
	{
		if (alguienEstuche)
		{
			numPiedras2 += minimo(piedrasEstuche, numPiedras1);
			numPiedras1 -= minimo(piedrasEstuche, numPiedras1);
		}
		if (jugadorTriunfo == 1 || jugadorTriunfo == 3)
		{
			...
			if (alguienSolo)
			{
				numPiedras2 += minimo(piedrasSolo, numPiedras1);
				numPiedras1 -= minimo(piedrasSolo, numPiedras1);
			}
		}
		else
		{
...
void Mesa::cambiaPorZanga(array<Jugador^>^ jugadores, int jugador)
{		
	cout << "El jugador " << jugador+1 << " pasa y cambia sus cartas por la zanga.\n";
	Mazo^ mazoTemp = gcnew Mazo();
	if (jugador == 0 || jugador == 2)
	{
		this->numPiedrasCentro += 1;
		this->numPiedras1 -= 1;
...
void Mesa::recambiaSusCartas(array<Jugador^>^ jugadores, int jugador)
{
...
void Mesa::comprobarEstuche(array<Jugador^>^ jugadores)
{
	int num1 = 0, num2 = 0;
