...
namespace ZangaGraficoDefinitivo {
	...
	public:
		...
		Form1(array<Jugador^>^ jugadores)
		{
			InitializeComponent();
			this->jugadores = gcnew array<Jugador^>(4);
			for (int i = 0; i < 4; i++)
			{
				this->jugadores[i] = gcnew Jugador(*(jugadores[i]));
			}
		}

		Form1(array<Jugador^>^ jugadores, bool soyServidor, TcpListener^ servidor, 
			array<TcpClient^>^ clientesConectados, TcpClient^ cliente, int jugadorServidor)
		{
			InitializeComponent();
			this->jugadores = gcnew array<Jugador^>(4);
			for (int i = 0; i < 4; i++)
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->botonComenzar->KeyPress += gcnew System::Windows::Forms::KeyPressEventHandler(this, &Form1::botonComenzar_KeyPress);
			// 
			// botonSalir
			// 
			...
			this->botonSalir->Click += 
				gcnew System::EventHandler(this, &ZangaGraficoDefinitivo::Form1::form1BotonMenuPrincipal_Click);
			// 
			// salida
			// 
			...
			// labelJugador2
			// 
			this->labelJugador2->BackColor = System::Drawing::SystemColors::Control;
...
#pragma endregion
	...
	private:
		...
		array<NetworkStream^>^ streams;
		array<unsigned char>^ bufferEscritura;
		array<unsigned char>^ bufferLectura;
		array<Object^>^ info;
		NetworkStream^ stream;
		Int32 bytesLeidos;
		Mesa^ mesa;
		array<Mazo^>^ mazosOriginales;
		int jugadorServidor;

		//// Parametros
		const static int tiempoMesa = 750;
		const static int tiempoJugadores = 1500;
		...
		void mainHiloJuego()
		{
			...
			mazosOriginales = gcnew array<Mazo^>
			{gcnew Mazo(), 	gcnew Mazo(), gcnew Mazo(), gcnew Mazo()};
			mesa = gcnew Mesa((jugadorServidor+1)%4);
			Carta^ comienzo = gcnew Carta();
			Carta^ cartaTemp = gcnew Carta();
			...
			// Inicialización variables online
			bufferLectura = gcnew array<unsigned char>(256);
			bufferEscritura = gcnew array<unsigned char>(256);
			streams = gcnew array<NetworkStream^>(3);
			int semilla;
			if (esOnline())
			{
				if (soyServidor) // Servidor
				{
					for (int i = 1; i < 4; i++)
					{
						if (esUsuario(i))
						{
							streams[jugadores[i]->getNumCliente()] = clientesConectados
								[jugadores[i]->getNumCliente()]->GetStream();
						}
					}
				}
				else // Cliente
			...
			if (esOnline() && !soyServidor)
			{
				bytesLeidos = stream->Read(bufferLectura, 0, bufferLectura->Length);
				if (bytesLeidos == 0 || 
					safe_cast<mensajesJuego>(bufferLectura[0]) != comenzarJuego)
				{
					escribirMensaje("Error: no se recibe nada del servidor");
					pausar();
				}
				info = gcnew array<Object^>{5};
				bufferEscritura = empaquetarMensaje(ack, info);
			...
			pausar();
			info = gcnew array<Object^>{5};
			bufferEscritura = empaquetarMensaje(comenzarJuego, info);						
			for (int i = 1; i < 4; i++)
			{
				if (esUsuario(i))
				 {
					 streams[jugadores[i]->getNumCliente()]->
						 Write(bufferEscritura, 0, bufferEscritura->Length);
				}
			}
			 for (int i = 1; i < 4; i++)
			 {
				 if (esUsuario(i))
				 {
					 bytesLeidos = streams[jugadores[i]->getNumCliente()]
					 ->Read(bufferLectura, 0, bufferLectura->Length);
					 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
						 || safe_cast<int>(bufferLectura[2]) != 5)
					 {
						 escribirMensaje("Error: no se recibe nada del jugador " +
			...
			{				
				...
				this->barajar(baraja, jugadorServidor, semilla);//jugadorServidor);
				esperar(tiempoMesa);
				this->cortar((jugadorServidor+3)%4);//(jugadorServidor+3)%4);
				esperar(tiempoMesa);
				mesa->repartir(baraja, jugadores);
				this->anunciarRepartir(jugadorServidor);
				resaltarJugador(jugadorServidor, false);
				this->actualizarCartasJugadores((jugadorServidor+1)%4);
				this->mostrarZanga(jugadorServidor);
				// Chapuza
				for (int i = 0; i < 4; i++)
				{
				...
				{
					comienzo = gcnew Carta((valores)indiceEspadas, espadas);
					for (int i = 0; i < 4; i++)
					{
						if (jugadores[i]->tieneCarta(comienzo))
						{					
							this->tieneEspadas(i, comienzo->nombre());
							esperar(tiempoMesa);		
							this->anunciarMano(i);
							mesa->setJugadorManoPrimero(i);							
							this->cambiarLabelProximoMano(
								jugadores[mesa->getJugadorManoPrimero()]->getNombre());
							esperar(tiempoMesa);
							alguienLaTiene = true;
							break;
						}
			...
			// Hablar			
			frasePrimero = obtenerFrasePrimero(mano);
			if (frasePrimero == juego)
			{
				this->juega(mano);
				fraseDespues = obtenerFraseDespues((mano+1)%4);
				if (fraseDespues == juegoTambien)
				{
					this->juega((mano+1)%4);					
					fraseDespues = obtenerFraseDespues((mano+2)%4);
					if (fraseDespues == juegoTambien)
					{
						this->juega((mano+2)%4);
						fraseDespues = obtenerFraseDespues((mano+3)%4);											
						if (fraseDespues == juegoTambien)
						{
							this->juega((mano+3)%4);
						}
						else
						{
							...
							aceptaSolo = obtenerAceptaSolo((mano+3)%4, (mano+0)%4);
							if (aceptaSolo)
							{
								this->vaSolo(mano, meVoyYo);
							}
							else
							{
								this->vaSolo((mano+3)%4, vayaUsted);									 
							}
						}
					}
					else
					{
						...
						aceptaSolo = obtenerAceptaSolo((mano+2)%4, (mano+0)%4);
						if (aceptaSolo)
						{
							this->vaSolo(mano, meVoyYo);
						}
						...
						{
							this->vaSolo((mano+2)%4, vayaUsted);
						}
					}
				} 
				else 
				{
					...
					aceptaSolo = obtenerAceptaSolo((mano+1)%4, (mano+0)%4);
					if (aceptaSolo)
					{
						this->vaSolo(mano, meVoyYo);
					}
					...
					{
						this->vaSolo((mano+1)%4, vayaUsted);
					}
				}
			} 
			...
			{
				this->pasa(mano);
				frasePrimero = obtenerFrasePrimero((mano+1)%4);
				if (frasePrimero == juego)
				{
					this->juega((mano+1)%4);
					fraseDespues = obtenerFraseDespues((mano+2)%4);
					if (fraseDespues == juegoTambien)
					{
						...
						else
						{
							...
							aceptaSolo = obtenerAceptaSolo((mano+3)%4, (mano+1)%4);
							if (aceptaSolo)
							{
								this->vaSolo((mano+1)%4, meVoyYo);
							}
							else
							{
								this->vaSolo((mano+3)%4, vayaUsted);
					...
					else
					{
						...
						aceptaSolo = obtenerAceptaSolo((mano+2)%4, (mano+1)%4);
						if (aceptaSolo)
						{
							this->vaSolo((mano+1)%4, meVoyYo);
						}
				...
				{
					this->pasa((mano+1)%4);
					frasePrimero = obtenerFrasePrimero((mano+2)%4);					
					if (frasePrimero == juego)
					{
						this->juega((mano+2)%4);
						fraseDespues = obtenerFraseDespues((mano+3)%4);
						...
						else
						{
							...
							aceptaSolo = obtenerAceptaSolo((mano+3)%4, (mano+2)%4);
							if (aceptaSolo)
							{
								this->vaSolo((mano+2)%4, meVoyYo);
							}
							else
							{
								this->vaSolo((mano+3)%4, vayaUsted);
					...
					{
						this->pasa((mano+2)%4);
						frasePrimero = obtenerFrasePrimero((mano+3)%4);
						if (frasePrimero == paso)
						{
							this->cambiaPorZanga((mano+3)%4);
							fraseZanga = obtenerFraseZanga((mano+3)%4);
							...
							{
								this->vaSolo((mano+3)%4, meVoy);	
							}
							else
							{
								this->recambiaSusCartas((mano+3)%4);								
								fraseRecambio = obtenerFraseRecambio((mano+3)%4);
					...
					{
						this->vaSolo((mano+2)%4, meVoy);
					}
				}
				else
				{
					this->vaSolo((mano+1)%4, meVoy);
				}
			}
			else
			{
				this->vaSolo(mano, meVoy);
			}		
			triunfo = obtenerTriunfo(mesa->getJugadorTriunfo(), (mano+3)%4);									
			this->setTriunfo(triunfo);
			resaltarJugador(0, false);
			resaltarJugador(1, false);
			resaltarJugador(2, false);
			resaltarJugador(3, false);
			/*if (usuarioJuega && !(mesa->getAlguienSolo() && mesa->getJugadorSolo() == 2))
			{
			jugadores[0]->ordenarMazo(mesa->getTriunfo());
			*mazosOriginales[0] = *(jugadores[0]->getMazo());
			actualizarCartasJugador(0);
			}*/

			esperar(tiempoMesa);
			this->cambiarLabelProximoMano(
				jugadores[(1+mesa->getJugadorManoPrimero())%4]->getNombre());
			...
			for (int i = 0; i < 8; i++)
			{
				...
				{					
					frasePideCinco = obtenerPideCinco(mano);
					if (frasePideCinco)
					{						
						this->pideCinco(mano);						
					}
				...
				{						
					temp = obtenerCartaElegida(mano, 0);
					*cartaTemp = *(jugadores[mano]->tirarCarta(temp));
					this->tirarCarta(mano, indiceOriginal(cartaTemp, mazosOriginales[mano]),
						cartaTemp, 0); 						

					temp = obtenerCartaElegida((mano+1)%4, 1);
					*cartaTemp = *(jugadores[(mano+1)%4]->tirarCarta(temp));
					this->tirarCarta((mano+1)%4, 
						indiceOriginal(cartaTemp, mazosOriginales[(mano+1)%4]), cartaTemp, 1);

					temp = obtenerCartaElegida((mano+2)%4, 2);
					*cartaTemp = *(jugadores[(mano+2)%4]->tirarCarta(temp));
					this->tirarCarta((mano+2)%4, 
						indiceOriginal(cartaTemp, mazosOriginales[(mano+2)%4]), cartaTemp, 2);

					temp = obtenerCartaElegida((mano+3)%4, 3);
					*cartaTemp = *(jugadores[(mano+3)%4]->tirarCarta(temp));
					this->tirarCarta((mano+3)%4, 
						indiceOriginal(cartaTemp, mazosOriginales[(mano+3)%4]), cartaTemp, 3);
				}
				else
				{				
					...
					if (jugadores[mano]->getNumero() != (mesa->getJugadorSolo()+2)%4)
					{
						...
						*cartaTemp = *(jugadores[mano]->tirarCarta(temp));
						this->tirarCarta(mano, indiceOriginal(cartaTemp, 
							mazosOriginales[mano]), cartaTemp, 0); 
					}
					...
					{				
						if (jugadores[mano]->getNumero() == (mesa->getJugadorSolo()+2)%4)
						{
							temp = obtenerCartaElegida((mano+1)%4, 0);
							*cartaTemp = *(jugadores[(mano+1)%4]->tirarCarta(temp));
							this->tirarCarta((mano+1)%4, indiceOriginal(cartaTemp, 
								mazosOriginales[(mano+1)%4]), cartaTemp, 0);
						}
						else
						{
							temp = obtenerCartaElegida((mano+1)%4, 1);							
							*cartaTemp = *(jugadores[(mano+1)%4]->tirarCarta(temp));
							this->tirarCarta((mano+1)%4, indiceOriginal(cartaTemp, 
								mazosOriginales[(mano+1)%4]), cartaTemp, 1);
						}
					}
					...
					{				
						...
						{
							temp = obtenerCartaElegida((mano+2)%4, 1);
							*cartaTemp = *(jugadores[(mano+2)%4]->tirarCarta(temp));
							this->tirarCarta((mano+2)%4, indiceOriginal(cartaTemp, 
								mazosOriginales[(mano+2)%4]), cartaTemp, 1);
						}
						else
						{
							temp = obtenerCartaElegida((mano+2)%4, 2);
				...
				if (mesa->getAlguienCinco())
				{
					...
					{					
						if (mesa->getNumBazas1() == 5 || mesa->getNumBazas2() == 5)
						{
							...
							this->cumpleCinco(true);
							esperar(tiempoMesa);
							frasePideTodas = obtenerPideTodas(mano);
							if (frasePideTodas)
							{
								this->pideTodas(mano);
			...
			while (true)
			{
				...
				esperar(tiempoMesa);
				semilla = obtenerSemilla();						
				this->barajar(baraja, (mesa->getJugadorManoPrimero()+3)%4, semilla);
				esperar(tiempoMesa);
				this->cortar((mesa->getJugadorManoPrimero()+2)%4);
				esperar(tiempoMesa);
				mesa->repartir(baraja, jugadores);
				this->anunciarRepartir((mesa->getJugadorManoPrimero()+3)%4);
				this->actualizarCartasJugadores(mesa->getJugadorManoPrimero());
			...
			this->anunciarVictoria(numManos, transcurrido);
		...
		void barajar(Baraja^ baraja, int jugador, int semilla) 
		{
			mesa->barajar(baraja, semilla);
			this->escribirMensaje(jugadores[jugador]->getNombre()
				+ " baraja las cartas.\n");
		}
		...
		void actualizarCartasJugadores(int mano)
		{
		...
		void actualizarCartasJugador(int jugador)
		{
			array<System::Windows::Forms::PictureBox^, 2>^cartasJugadores = {
				{cartaAbajo0, cartaAbajo1, cartaAbajo2, cartaAbajo3, 
				cartaAbajo4, cartaAbajo5, cartaAbajo6, cartaAbajo7}, 
		...
		void anunciarMano(int jugadorManoPrimero) 
		{
			int piedras;
			...
			if (jugadorManoPrimero == 0 || jugadorManoPrimero == 2)	
			{
				...
				if (piedras >= 4)
				{
					this->escribirMensaje(jugadores[jugadorManoPrimero]->getNombre()
		...
		{
			this->escribirMensaje(jugadores[jugador]->getNombre() + " pasa y mete un vale.\n");
			...
			{
				System::Media::SoundPlayer sonidoPaso("paso0.wav");
				sonidoPaso.Play();
			}
			mesa->pasa(jugador);
		}
		...
		{			
			this->escribirMensaje(jugadores[jugador]->getNombre() + " juega.\n");
			if (reproducirSonidos)
			{
				System::Media::SoundPlayer sonidoJuego("juego0.wav");		
				sonidoJuego.Play();
			}
			mesa->juega(jugador);
		}
		...
		{
			this->escribirMensaje(jugadores[mandador]->getNombre() + " manda solo a" + 
				jugadores[mandado]->getNombre()+ ".\n");
			if (reproducirSonidos)
			{
				System::Media::SoundPlayer sonidoVayaSolo("vayaSolo0.wav");		
				sonidoVayaSolo.Play();				
			}
			mesa->mandaSolo(mandador, mandado);
		}
		...
		void vaSolo(int jugador, formasSolo forma)
		{
			...
			{
			case meVoy:
				this->escribirMensaje(jugadores[jugador]->getNombre() + " se va solo.\n");
				if (reproducirSonidos)
				{
					System::Media::SoundPlayer sonidoMeVoySolo("meVoySolo0.wav");
					sonidoMeVoySolo.Play();				
				}
				break;
			case meVoyYo:
				this->escribirMensaje(jugadores[jugador]->getNombre() + 
				...
				break;
			default:
				this->escribirMensaje(jugadores[jugadorTriunfo]->getNombre() + 
					" no acepta y " + jugadores[jugador]->getNombre() + " se va solo.\n");
				if (reproducirSonidos)
				{
					System::Media::SoundPlayer sonidoVayaUsted("vayaUsted0.wav");	
					sonidoVayaUsted.Play();
				}
			}		
			this->cambiarSolo(true);
			this->limpiarCartasJugador((jugador+2)%4);	
			mesa->vaSolo(jugador, forma);
		}

		void cambiaPorZanga(int jugador)
		{		
			this->escribirMensaje(jugadores[jugador]->getNombre()
			...
			mesa->cambiaPorZanga(jugadores, jugador);
			esperar(tiempoMesa);
			this->actualizarCartasJugador(jugador);
			*(mazosOriginales[jugador]) = *(jugadores[jugador]->getMazo());
		}

		void recambiaSusCartas(int jugador)
		{
			this->escribirMensaje(jugadores[jugador]->getNombre() +
			...
			if (reproducirSonidos)
			{
				System::Media::SoundPlayer sonidoRecambiarCartas("cojoMisCartas0.wav");		
				sonidoRecambiarCartas.Play();
			}
			mesa->recambiaSusCartas(jugadores, jugador);
			esperar(tiempoMesa);
			this->actualizarCartasJugador(jugador);
			*(mazosOriginales[jugador]) = *(jugadores[jugador]->getMazo());
		}
		...
		void setTriunfo(palos triunfo)
		{
			...
			this->cambiarLabelTriunfa(jugadores[mesa->getJugadorTriunfo()]->getNombre());
		}
		...
		void pideCinco(int jugador)
		{
			...
			if (reproducirSonidos)
			{
				System::Media::SoundPlayer sonidoCinco("cinco0.wav");
				sonidoCinco.Play();
			}
			System::Media::SoundPlayer sonidoTodas("todas0.wav");	
			mesa->pideCinco(jugador);
		}
		...
		void tirarCarta(int jugador, int numCarta, Carta^ carta, int turno)
		{
		...
		void organizarBaza() 
		{
			...
			this->escribirMensaje("Baza ganada por " 
				+ jugadores[mesa->getJugadorMano()]->getNombre() + ".\n");
			this->limpiarTapete();
		}
		...
		void organizarPiedras()
		{
			...
			if (mesa->getAlguienCinco())
			{
				if (mesa->getJugadorCinco() == 0 || mesa->getJugadorCinco() == 2)
				{
					...
					{
						this->ponerPiedras1(minimo(mesa->getPiedrasCinco(), piedrasGraficas2));
						this->quitarPiedras2(minimo(mesa->getPiedrasCinco(), piedrasGraficas2));
						this->escribirMensaje("La pareja 1 ha conseguido Cinco y cobra "
							+ minimo(mesa->getPiedrasCinco(), piedrasGraficas2) + " vales.\n");
					}
					else
					{
						this->ponerPiedras2(minimo(mesa->getPiedrasCinco(), piedrasGraficas1));
						this->quitarPiedras1(minimo(mesa->getPiedrasCinco(), piedrasGraficas1));
						this->escribirMensaje("La pareja 1 no ha conseguido Cinco y paga "
							+ minimo(mesa->getPiedrasCinco(), piedrasGraficas1) + " vales.\n");
					}
				}
				else
				{
			...
			if (mesa->getAlguienTodas())
			{
				if (mesa->getJugadorTodas() == 0 || mesa->getJugadorTodas() == 2)
				{
					...
					{
						this->ponerPiedras1(minimo(mesa->getPiedrasTodas(), piedrasGraficas2));
						this->quitarPiedras2(minimo(mesa->getPiedrasTodas(), piedrasGraficas2));
						this->escribirMensaje("La pareja 1 ha conseguido Todas y cobra "
							+ minimo(mesa->getPiedrasTodas(), piedrasGraficas2) + " vales.\n");
					}
					else
					{
						this->ponerPiedras2(minimo(mesa->getPiedrasTodas(), piedrasGraficas1));
						this->quitarPiedras1(minimo(mesa->getPiedrasTodas(), piedrasGraficas1));
						this->escribirMensaje("La pareja 1 no ha conseguido Todas y paga "
							+ minimo(mesa->getPiedrasTodas(), piedrasGraficas1) + " vales.\n");
					}
				}
				else
				{
			...
			if (mesa->getNumBazas1() > mesa->getNumBazas2())
			{
				...
				{
					this->ponerPiedras1(minimo(mesa->getPiedrasEstuche(), piedrasGraficas2));
					this->quitarPiedras2(minimo(mesa->getPiedrasEstuche(), piedrasGraficas2));
				...
				if (mesa->getJugadorTriunfo() == 0 || mesa->getJugadorTriunfo() == 2)
				{
					...
					if (mesa->getAlguienSolo())
					{
						this->ponerPiedras1(minimo(mesa->getPiedrasSolo(), piedrasGraficas2));
						this->quitarPiedras2(minimo(mesa->getPiedrasSolo(), piedrasGraficas2));
						this->escribirMensaje("" + jugadores[mesa->getJugadorSolo()]->getNombre()
							+ " ha ganado por " 
				...
				else
				{
					if (mesa->getAlguienSolo())
					{
						...
						this->escribirMensaje("La pareja 1 ha ganado por " + mesa->getNumBazas1() + " a "
							...
							+ " porque " + 
							jugadores[mesa->getJugadorSolo()]->getNombre() + " se fue solo.\n");
					}
					else
						this->escribirMensaje("La pareja 1 ha ganado por " + mesa->getNumBazas1() + " a "
			...
			else if (mesa->getNumBazas2() > mesa->getNumBazas1())
			{
				if (mesa->getAlguienEstuche())
				{
					this->ponerPiedras2(minimo(mesa->getPiedrasEstuche(), piedrasGraficas1));
					this->quitarPiedras1(minimo(mesa->getPiedrasEstuche(), piedrasGraficas1));
				...
				if (mesa->getJugadorTriunfo() == 1 || mesa->getJugadorTriunfo() == 3)
				{
					...
					if (mesa->getAlguienSolo())
					{
						this->ponerPiedras2(minimo(mesa->getPiedrasSolo(), piedrasGraficas1));
						this->quitarPiedras1(minimo(mesa->getPiedrasSolo(), piedrasGraficas1));
				...
				else
				{
					if (mesa->getAlguienSolo())
					{
						...
						this->escribirMensaje("La pareja 2 ha ganado por " + mesa->getNumBazas1() + " a "
							+ mesa->getNumBazas2() + ". Ha sido Codillo y cobra " + mesa->getPiedrasSolo()
							+ " porque " + jugadores[mesa->getJugadorSolo()]->getNombre()
							+ " se fue solo.\n");
					}
					else
					{
		...
		void anunciarVictoria(int numManos, System::TimeSpan^ tiempo)
		{
		...
		void activarBotonesHablaPrimero()
		{
			situacion = primero;
		...
		void activarCartas(int turno)
		{
			...
			else
			{
				for (int i = 0; i < 8; i++)
				{
					if (cartasNoTiradas[0, i])
					{
						if (jugadores[0]->esLegal(mazosOriginales[0]->getCarta(i), 
		...
		//		cartaAbajo0, cartaAbajo1, cartaAbajo2, cartaAbajo3, 
		//		cartaAbajo4, cartaAbajo5, cartaAbajo6, cartaAbajo7};
		//		System::Drawing::Rectangle bordes = carta->Bounds;
		...
		bool esUsuario(int jugador)
		{
			return jugadores[jugador]->getAlgoritmoIA() == 10;
		}

		bool esOnline()
		{
			return esUsuario(1) || esUsuario(2) || esUsuario(3);
		}

		array<unsigned char>^ empaquetarMensaje(mensajesJuego tipoMensaje, 
			array<Object^>^ info)
		{
			int temp;
			array<unsigned char>^ msj;
			msj = gcnew array<unsigned char>(3);
			msj[0] = safe_cast<unsigned char>(tipoMensaje);
			msj[1] = safe_cast<unsigned char>(1+1+1);
			switch(tipoMensaje)
			{
			case aceptarSoloMensaje:
			case hablarCinco:
			case hablarTodas:
				temp = safe_cast<int>(System::Convert::ToInt16(info[0]));
				msj[2] = safe_cast<unsigned char>(temp);
				break;
			default:
				//temp = safe_cast<int>(info[0]);
				temp = safe_cast<int>(System::Convert::ToInt16(info[0]));
				msj[2] = safe_cast<unsigned char>(temp);
			}
		...
		System::Void botonComenzar_Click(System::Object^  sender, System::EventArgs^  e) 
		{
			...
			this->ocultarBarajas();
			this->ocultarBarajasCortar();*/	
			//escribirMensaje("TURURU");
			botonComenzar->Enabled = false;
			reanudar();
		}
	...
	public: System::Void ZangaGraficoDefinitivo::Form1::form1BotonMenuPrincipal_Click
	...
	private: System::Void Form1_Load(System::Object^  sender, System::EventArgs^  e) 
			 {
				 ...
				 situacion = primero;
				 hiloSuspendido = false;
				 cambiarLabel(labelJugador0, jugadores[0]->getNombre());
				 cambiarLabel(labelJugador1, jugadores[1]->getNombre());
				 cambiarLabel(labelJugador2, jugadores[2]->getNombre());
				 cambiarLabel(labelJugador3, jugadores[3]->getNombre());
				 if (esOnline() && !soyServidor)
				 {
					 botonComenzar->Enabled = false;					 
				 }
	...
	private: System::Void cartaAbajo0_Click(System::Object^  sender, System::EventArgs^  e) 
			 {
				 if (esUsuario(0) && cartasActivas[0])
				 {
					 usuarioCartaElegida = 0;
					 desactivarCartas();
					 reanudar();
	...
	private: System::Void cartaAbajo1_Click(System::Object^  sender, System::EventArgs^  e) 
			 {
				 if (esUsuario(0) && cartasActivas[1])
				 {
	...
	private: System::Void cartaAbajo2_Click(System::Object^  sender, System::EventArgs^  e) {
				 if (esUsuario(0) && cartasActivas[2])
				 {
	...
	private: System::Void cartaAbajo3_Click(System::Object^  sender, System::EventArgs^  e) {
				 if (esUsuario(0) && cartasActivas[3])
				 {
	...
	private: System::Void cartaAbajo4_Click(System::Object^  sender, System::EventArgs^  e) {
				 if (esUsuario(0) && cartasActivas[4])
				 {
	...
	private: System::Void cartaAbajo5_Click(System::Object^  sender, System::EventArgs^  e) {
				 if (esUsuario(0) && cartasActivas[5])
				 {
	...
	private: System::Void cartaAbajo6_Click(System::Object^  sender, System::EventArgs^  e) {
				 if (esUsuario(0) && cartasActivas[6])
				 {
	...
	private: System::Void cartaAbajo7_Click(System::Object^  sender, System::EventArgs^  e) {
				 if (esUsuario(0) && cartasActivas[7])
				 {
	...
	private: System::Void Form1_Paint(System::Object^  sender, System::Windows::Forms::PaintEventArgs^  e) 
			 ...
			 System::Void botonEspadas_Click(System::Object^  sender, System::EventArgs^  e) 
			 {
				 usuarioTriunfo = espadas;
				 desactivarBotones();
				 reanudar();
			 }
			 System::Void botonBastos_Click(System::Object^  sender, System::EventArgs^  e) {
				 usuarioTriunfo = bastos;
				 desactivarBotones();
				 reanudar();
			 }
			 System::Void botonOros_Click(System::Object^  sender, System::EventArgs^  e) {
				 usuarioTriunfo = oros;
				 desactivarBotones();
				 reanudar();
			 }
			 System::Void botonCopas_Click(System::Object^  sender, System::EventArgs^  e) {
			 ...
			 System::Void botonComenzar_KeyPress(System::Object^  sender, System::Windows::Forms::KeyPressEventArgs^  e) 
			 {
				 if (e->KeyChar == 'x')
				 {
					 if (hiloSuspendido == false)
			 ...
			 {
				 int semilla = 0;
				 if (esUsuario(0))  // Alguien juega
				 {
					 if (esOnline()) // Es online
					 {
						 if (soyServidor) // Servidor
						 {
							 // Genero la semilla
							 semilla = System::DateTime::Now.Millisecond % 256;
							 // La envío a todos los clientes conectados
							 info = gcnew array<Object^>{(int)semilla};
							 bufferEscritura = empaquetarMensaje(semillaBaraja, info);
							 for (int i = 1; i < 4; i++)
							 {
								 if (esUsuario(i))
								 {									
							 ...
							 // Recibo el ACK de todos los clientes conectados
							 for (int i = 1; i < 4; i++)
							 {
								 if (esUsuario(i))
								 {									
									 ...
									 ->Read(bufferLectura, 0, bufferLectura->Length);
									 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
										 || safe_cast<int>(bufferLectura[2]) != semilla)
									 {
										 escribirMensaje("Error: no se recibe nada de " +
											 jugadores[i]->getNombre());
										 pausar();
									 }
								 }
						 ...
						 {
							 // Recibo la semilla procedente del servidor
							 bytesLeidos = stream->Read(bufferLectura, 0, bufferLectura->Length);
							 if (bytesLeidos > 0)
							 {
								 semilla = safe_cast<int>(bufferLectura[2]);
								 // Vuelvo a empaquetar la semillo y la envío como ACK
								 info = gcnew array<Object^>{(int)semilla};
								 bufferEscritura = empaquetarMensaje(ack, info);
								 stream->Write(bufferEscritura, 0, bufferEscritura->Length);
							 }
			 ...
			 frasesHablarPrimero obtenerFrasePrimero(int jugador)
			 {
				 resaltarJugador((jugador+0)%4, true);
				 resaltarJugador((jugador+3)%4, false);
				 frasesHablarPrimero frasePrimero;
				 if (esUsuario(0)) // Alguien juega
				 {
					 if (esOnline()) // Es online
					 {
						 if (soyServidor) // Servidor
						 {
							 if (jugador == 0) // Me toca a mi
							 {
								 // El usuario elige
								 activarBotonesHablaPrimero();
								 pausar();
								 // Empaqueto lo que elijo y lo envío a todos los clientes conectados
								 info = gcnew array<Object^>{(int)usuarioFrasePrimero};
								 bufferEscritura = empaquetarMensaje(hablarPrimero, info);
								 for (int i = 1; i < 4; i++)
								 {
									 if (esUsuario(i))
									 {									
								 ...
								 // Recibo los ACK de todos los clientes conectados
								 for (int i = 1; i < 4; i++)
								 {
									 if (esUsuario(i))
									 {									
										 bytesLeidos = streams[jugadores[i]->getNumCliente()]->Read(bufferLectura, 0, bufferLectura->Length);
										 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
											 || safe_cast<frasesHablarPrimero>(bufferLectura[2]) != usuarioFrasePrimero)
										 {
											 escribirMensaje("Error: no se recibe nada de" +
												 jugadores[i]->getNombre());
											 pausar();
							 ...
							 else // No me toca a mi
							 {
								 if (esUsuario(jugador)) // Es un cliente
								 {
									 // Recibo su elección
									 bytesLeidos = streams[jugadores[jugador]->getNumCliente()]
									 ->Read(bufferLectura, 0, bufferLectura->Length);
									 if (bytesLeidos > 0)
									 {
										 usuarioFrasePrimero = safe_cast<frasesHablarPrimero>(bufferLectura[2]);
										 // Le envío el ACK con su elección
										 info = gcnew array<Object^>{(int)usuarioFrasePrimero};
										 bufferEscritura = empaquetarMensaje(ack, info);
										 streams[jugadores[jugador]->getNumCliente()]
										 ->Write(bufferEscritura, 0, bufferEscritura->Length);
										 // Le envío a los demás clientes su elección
										 bufferEscritura = empaquetarMensaje(hablarPrimero, info);
										 for (int i = 1; i < 4 && i != jugador; i++)
										 {
											 if (esUsuario(i))
											 {
												 streams[jugadores[i]->getNumCliente()]
										 ...
										 // Recibo el ACK de los demás clientes
										 for (int i = 1; i < 4 && i != jugador; i++)
										 {
											 if (esUsuario(i))
											 {
						 ...
						 else // Cliente
						 {
							 if (jugador == 0) // Me toca a mi
							 {
								 ...
								 activarBotonesHablaPrimero();
								 pausar();
								 // Empaqueto lo que elijo y lo envío al servidor
								 info = gcnew array<Object^>{(int)usuarioFrasePrimero};
								 bufferEscritura = empaquetarMensaje(hablarPrimero, info);
								 stream->Write(bufferEscritura, 0, bufferEscritura->Length);
								 // Recibo el ack del servidor
								 bytesLeidos = stream->Read(bufferLectura, 0, bufferLectura->Length);
								 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
									 || safe_cast<frasesHablarPrimero>(bufferLectura[2]) != usuarioFrasePrimero)
								 {
							 ...
							 {
								 // Recibo del servidor la elección del jugador
								 bytesLeidos = stream->Read(bufferLectura, 0, bufferLectura->Length);
								 if (bytesLeidos > 0)
								 {
									 usuarioFrasePrimero = safe_cast<frasesHablarPrimero>(bufferLectura[2]);
									 // Envío el ACK con la elección
									 info = gcnew array<Object^>{(int)usuarioFrasePrimero};
									 bufferEscritura = empaquetarMensaje(ack, info);
									 stream->Write(bufferEscritura, 0, bufferEscritura->Length);
								 }
				 ...
				 frasePrimero = jugadores[jugador]->hablarPrimero(mesa, usuarioFrasePrimero);

				 if (esUsuario(0) && esOnline() && soyServidor && jugador != 0 && !esUsuario(jugador)) 
					 // Tengo que pasarles a los demás lo que ha tirado el Ordenador
				 {
					 info = gcnew array<Object^>{(int)frasePrimero};
					 bufferEscritura = empaquetarMensaje(hablarPrimero, info);
					 for (int i = 1; i < 4; i++)
					 {
						 if (esUsuario(i))
					 ...
					 for (int i = 1; i < 4; i++)
					 {
						 ...
						 {									
							 ...
							 ->Read(bufferLectura, 0, bufferLectura->Length);
							 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
								 || safe_cast<frasesHablarPrimero>(bufferLectura[2]) != frasePrimero)
							 {
								 escribirMensaje("Error: no se recibe nada del jugador " +
									 jugadores[i]->getNombre());
								 pausar();
			 ...
			 frasesHablarDespues obtenerFraseDespues(int jugador)
			 {
				 resaltarJugador((jugador+0)%4, true);
				 resaltarJugador((jugador+3)%4, false);
				 frasesHablarDespues fraseDespues;
					 ...
					 if (esOnline()) // Es online
					 {
						 if (soyServidor) // Servidor
						 {
							 if (jugador == 0) // Me toca a mi
							 {
								 ...
								 pausar();
								 // Empaqueto lo que elijo y lo envío a todos los clientes conectados
								 info = gcnew array<Object^>{(int)usuarioFraseDespues};
								 bufferEscritura = empaquetarMensaje(hablarDespues, info);
								 for (int i = 1; i < 4; i++)
								 {
									 if (esUsuario(i))
									 {									
								 ...
								 for (int i = 1; i < 4; i++)
								 {
									 ...
									 {									
										 bytesLeidos = streams[jugadores[i]->getNumCliente()]->Read(bufferLectura, 0, bufferLectura->Length);
										 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
											 || safe_cast<frasesHablarDespues>(bufferLectura[2]) != usuarioFraseDespues)
										 {
											 escribirMensaje("Error: no se recibe nada del jugador " +
												 jugadores[i]->getNombre());
											 pausar();
							 ...
							 else // No me toca a mi
							 {
								 if (esUsuario(jugador)) // Es un cliente
								 {
									 ...
									 if (bytesLeidos > 0)
									 {
										 usuarioFraseDespues = safe_cast<frasesHablarDespues>(bufferLectura[2]);
										 // Le envío el ACK con su elección
										 info = gcnew array<Object^>{(int)usuarioFraseDespues};
										 bufferEscritura = empaquetarMensaje(ack, info);
										 streams[jugadores[jugador]->getNumCliente()]
				 ...
				 fraseDespues = jugadores[jugador]->hablarDespues(mesa, usuarioFraseDespues);

				 if (esUsuario(0) && esOnline() && soyServidor && jugador != 0 && !esUsuario(jugador)) 
					 // Tengo que pasarles a los demás lo que ha tirado el Ordenador
				 {
					 info = gcnew array<Object^>{(int)fraseDespues};
					 bufferEscritura = empaquetarMensaje(hablarDespues, info);
					 for (int i = 1; i < 4; i++)
					 {
						 if (esUsuario(i))
					 ...
					 for (int i = 1; i < 4; i++)
					 {
						 ...
						 {									
							 ...
							 ->Read(bufferLectura, 0, bufferLectura->Length);
							 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
								 || safe_cast<frasesHablarDespues>(bufferLectura[2]) != fraseDespues)
							 {
								 escribirMensaje("Error: no se recibe nada del jugador " +
									 jugadores[i]->getNombre());
								 pausar();
			 ...
			 bool obtenerAceptaSolo(int mandador, int jugador)
			 {
				 resaltarJugador((jugador+0)%4, true);
				 resaltarJugador(mandador, false);
				 bool fraseAceptaSolo = false;
				 if (esUsuario(0)) // Alguien juega
				 {
					 if (esOnline()) // Es online
					 {
						 if (soyServidor) // Servidor
						 {
							 if (jugador == 0) // Me toca a mi
							 {
								 ...
								 pausar();
								 // Empaqueto lo que elijo y lo envío a todos los clientes conectados
								 info = gcnew array<Object^>{usuarioAceptaSolo};
								 bufferEscritura = empaquetarMensaje(aceptarSoloMensaje, info);
								 for (int i = 1; i < 4; i++)
								 {
									 if (esUsuario(i))
									 {									
								 ...
								 for (int i = 1; i < 4; i++)
								 {
									 ...
									 {									
										 bytesLeidos = streams[jugadores[i]->getNumCliente()]->Read(bufferLectura, 0, bufferLectura->Length);
										 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
											 || safe_cast<bool>(bufferLectura[2]) != usuarioAceptaSolo)
										 {
											 escribirMensaje("Error: no se recibe nada del jugador " +
												 jugadores[i]->getNombre());
											 pausar();
							 ...
							 else // No me toca a mi
							 {
								 if (esUsuario(jugador)) // Es un cliente
								 {
									 ...
									 if (bytesLeidos > 0)
									 {
										 usuarioAceptaSolo = safe_cast<bool>(bufferLectura[2]);
										 // Le envío el ACK con su elección
										 info = gcnew array<Object^>{usuarioAceptaSolo};
										 bufferEscritura = empaquetarMensaje(ack, info);
										 streams[jugadores[jugador]->getNumCliente()]
				 ...
				 fraseAceptaSolo = jugadores[jugador]->aceptarSolo(mandador, mesa, usuarioAceptaSolo);

				 if (esUsuario(0) && esOnline() && soyServidor && jugador != 0 && !esUsuario(jugador)) 
					 // Tengo que pasarles a los demás lo que ha tirado el Ordenador
				 {
					 info = gcnew array<Object^>{fraseAceptaSolo};
					 bufferEscritura = empaquetarMensaje(aceptarSoloMensaje, info);
					 for (int i = 1; i < 4; i++)
					 {
						 if (esUsuario(i))
					 ...
					 for (int i = 1; i < 4; i++)
					 {
						 ...
						 {									
							 ...
							 ->Read(bufferLectura, 0, bufferLectura->Length);
							 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
								 || safe_cast<bool>(bufferLectura[2]) != fraseAceptaSolo)
							 {
								 escribirMensaje("Error: no se recibe nada del jugador " +
									 jugadores[i]->getNombre());
								 pausar();
				 ...
				 return fraseAceptaSolo;
			 }

			 frasesHablarZanga obtenerFraseZanga(int jugador)
			 {
				 resaltarJugador((jugador+0)%4, true);
				 resaltarJugador((jugador+3)%4, false);
				 frasesHablarZanga fraseZanga;
					 ...
					 if (esOnline()) // Es online
					 {
						 if (soyServidor) // Servidor
						 {
							 if (jugador == 0) // Me toca a mi
							 {
								 ...
								 pausar();
								 // Empaqueto lo que elijo y lo envío a todos los clientes conectados
								 info = gcnew array<Object^>{(int)usuarioFraseZanga};
								 bufferEscritura = empaquetarMensaje(hablarZanga, info);
								 for (int i = 1; i < 4; i++)
								 {
									 if (esUsuario(i))
									 {									
								 ...
								 for (int i = 1; i < 4; i++)
								 {
									 ...
									 {									
										 bytesLeidos = streams[jugadores[i]->getNumCliente()]->Read(bufferLectura, 0, bufferLectura->Length);
										 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
											 || safe_cast<frasesHablarZanga>(bufferLectura[2]) != usuarioFraseZanga)
										 {
											 escribirMensaje("Error: no se recibe nada del jugador " +
												 jugadores[i]->getNombre());
											 pausar();
							 ...
							 else // No me toca a mi
							 {
								 if (esUsuario(jugador)) // Es un cliente
								 {
									 ...
									 if (bytesLeidos > 0)
									 {
										 usuarioFraseZanga = safe_cast<frasesHablarZanga>(bufferLectura[2]);
										 // Le envío el ACK con su elección
										 info = gcnew array<Object^>{(int)usuarioFraseZanga};
										 bufferEscritura = empaquetarMensaje(ack, info);
										 streams[jugadores[jugador]->getNumCliente()]
				 ...
				 fraseZanga = jugadores[jugador]->hablarZanga(mesa, usuarioFraseZanga);

				 if (esUsuario(0) && esOnline() && soyServidor && jugador != 0 && !esUsuario(jugador)) 
					 // Tengo que pasarles a los demás lo que ha tirado el Ordenador
				 {
					 info = gcnew array<Object^>{(int)fraseZanga};
					 bufferEscritura = empaquetarMensaje(hablarZanga, info);
					 for (int i = 1; i < 4; i++)
					 {
						 if (esUsuario(i))
					 ...
					 for (int i = 1; i < 4; i++)
					 {
						 ...
						 {									
							 ...
							 ->Read(bufferLectura, 0, bufferLectura->Length);
							 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
								 || safe_cast<frasesHablarZanga>(bufferLectura[2]) != fraseZanga)
							 {
								 escribirMensaje("Error: no se recibe nada del jugador " +
									 jugadores[i]->getNombre());
								 pausar();
			 ...
			 frasesHablarRecambio obtenerFraseRecambio(int jugador)
			 {
				 resaltarJugador((jugador+0)%4, true);
				 resaltarJugador((jugador+3)%4, false);
				 frasesHablarRecambio fraseRecambio;
					 ...
					 if (esOnline()) // Es online
					 {
						 if (soyServidor) // Servidor
						 {
							 if (jugador == 0) // Me toca a mi
							 {
								 ...
								 pausar();
								 // Empaqueto lo que elijo y lo envío a todos los clientes conectados
								 info = gcnew array<Object^>{(int)usuarioFraseRecambio};
								 bufferEscritura = empaquetarMensaje(hablarRecambio, info);
								 for (int i = 1; i < 4; i++)
								 {
									 if (esUsuario(i))
									 {									
								 ...
								 for (int i = 1; i < 4; i++)
								 {
									 ...
									 {									
										 bytesLeidos = streams[jugadores[i]->getNumCliente()]->Read(bufferLectura, 0, bufferLectura->Length);
										 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
											 || safe_cast<frasesHablarRecambio>(bufferLectura[2]) != usuarioFraseRecambio)
										 {
											 escribirMensaje("Error: no se recibe nada del jugador " +
												 jugadores[i]->getNombre());
											 pausar();
							 ...
							 else // No me toca a mi
							 {
								 if (esUsuario(jugador)) // Es un cliente
								 {
									 ...
									 if (bytesLeidos > 0)
									 {
										 usuarioFraseRecambio = safe_cast<frasesHablarRecambio>(bufferLectura[2]);
										 // Le envío el ACK con su elección
										 info = gcnew array<Object^>{(int)usuarioFraseRecambio};
										 bufferEscritura = empaquetarMensaje(ack, info);
										 streams[jugadores[jugador]->getNumCliente()]
				 ...
				 fraseRecambio = jugadores[jugador]->hablarRecambio(mesa, usuarioFraseRecambio);

				 if (esUsuario(0) && esOnline() && soyServidor && jugador != 0 && !esUsuario(jugador)) 
					 // Tengo que pasarles a los demás lo que ha tirado el Ordenador
				 {
					 info = gcnew array<Object^>{(int)fraseRecambio};
					 bufferEscritura = empaquetarMensaje(hablarRecambio, info);
					 for (int i = 1; i < 4; i++)
					 {
						 if (esUsuario(i))
					 ...
					 for (int i = 1; i < 4; i++)
					 {
						 ...
						 {									
							 ...
							 ->Read(bufferLectura, 0, bufferLectura->Length);
							 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
								 || safe_cast<frasesHablarRecambio>(bufferLectura[2]) != fraseRecambio)
							 {
								 escribirMensaje("Error: no se recibe nada del jugador " +
									 jugadores[i]->getNombre());
								 pausar();
				 ...
				 return fraseRecambio;
			 }

			 palos obtenerTriunfo(int jugador, int ultimo)
			 {
				 resaltarJugador(jugador, true);
				 resaltarJugador(ultimo, false);
				 palos triunfo;
				 if (esUsuario(0)) // Alguien juega
				 {
					 if (esOnline()) // Es online
					 {
						 if (soyServidor) // Servidor
						 {
							 if (jugador == 0) // Me toca a mi
							 {
								 ...
								 pausar();
								 // Empaqueto lo que elijo y lo envío a todos los clientes conectados
								 info = gcnew array<Object^>{(int)usuarioTriunfo};
								 bufferEscritura = empaquetarMensaje(hablarTriunfo, info);
								 for (int i = 1; i < 4; i++)
								 {
									 if (esUsuario(i))
									 {									
								 ...
								 for (int i = 1; i < 4; i++)
								 {
									 ...
									 {									
										 bytesLeidos = streams[jugadores[i]->getNumCliente()]->Read(bufferLectura, 0, bufferLectura->Length);
										 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
											 || safe_cast<palos>(bufferLectura[2]) != usuarioTriunfo)
										 {
											 escribirMensaje("Error: no se recibe nada del jugador " +
												 jugadores[i]->getNombre());
											 pausar();
							 ...
							 else // No me toca a mi
							 {
								 if (esUsuario(jugador)) // Es un cliente
								 {
									 ...
									 if (bytesLeidos > 0)
									 {
										 usuarioTriunfo = safe_cast<palos>(bufferLectura[2]);
										 // Le envío el ACK con su elección
										 info = gcnew array<Object^>{(int)usuarioTriunfo};
										 bufferEscritura = empaquetarMensaje(ack, info);
										 streams[jugadores[jugador]->getNumCliente()]
				 ...
				 triunfo = jugadores[jugador]->triunfar(usuarioTriunfo);

				 if (esUsuario(0) && esOnline() && soyServidor && jugador != 0 && !esUsuario(jugador)) 
					 // Tengo que pasarles a los demás lo que ha tirado el Ordenador
				 {
					 info = gcnew array<Object^>{(int)triunfo};
					 bufferEscritura = empaquetarMensaje(hablarTriunfo, info);
					 for (int i = 1; i < 4; i++)
					 {
						 if (esUsuario(i))
					 ...
					 for (int i = 1; i < 4; i++)
					 {
						 ...
						 {									
							 ...
							 ->Read(bufferLectura, 0, bufferLectura->Length);
							 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
								 || safe_cast<palos>(bufferLectura[2]) != triunfo)
							 {
								 escribirMensaje("Error: no se recibe nada del jugador " +
									jugadores[i]->getNombre());
								 pausar();
			 ...
			 bool obtenerPideCinco(int jugador)
			 {
				 resaltarJugador((jugador+0)%4, true);
				 resaltarJugador((jugador+3)%4, false);
				 bool frasePideCinco = false;
				 if (esUsuario(0)) // Alguien juega
				 {
					 if (esOnline()) // Es online
					 {
						 if (soyServidor) // Servidor
						 {
							 if (jugador == 0) // Me toca a mi
							 {
								 ...
								 pausar();
								 // Empaqueto lo que elijo y lo envío a todos los clientes conectados
								 info = gcnew array<Object^>{usuarioPideCinco};
								 bufferEscritura = empaquetarMensaje(hablarCinco, info);
								 for (int i = 1; i < 4; i++)
								 {
									 if (esUsuario(i))
									 {									
								 ...
								 for (int i = 1; i < 4; i++)
								 {
									 ...
									 {									
										 bytesLeidos = streams[jugadores[i]->getNumCliente()]->Read(bufferLectura, 0, bufferLectura->Length);
										 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
											 || safe_cast<bool>(bufferLectura[2]) != usuarioPideCinco)
										 {
											 escribirMensaje("Error: no se recibe nada del jugador " +
												 jugadores[i]->getNombre());
											 pausar();
							 ...
							 else // No me toca a mi
							 {
								 if (esUsuario(jugador)) // Es un cliente
								 {
									 ...
									 if (bytesLeidos > 0)
									 {
										 usuarioPideCinco = safe_cast<bool>(bufferLectura[2]);
										 // Le envío el ACK con su elección
										 info = gcnew array<Object^>{usuarioPideCinco};
										 bufferEscritura = empaquetarMensaje(ack, info);
										 streams[jugadores[jugador]->getNumCliente()]
				 ...
				 frasePideCinco = jugadores[jugador]->pedirCinco(mesa, usuarioPideCinco);

				 if (esUsuario(0) && esOnline() && soyServidor && jugador != 0 && !esUsuario(jugador)) 
					 // Tengo que pasarles a los demás lo que ha tirado el Ordenador
				 {
					 info = gcnew array<Object^>{frasePideCinco};
					 bufferEscritura = empaquetarMensaje(hablarCinco, info);
					 for (int i = 1; i < 4; i++)
					 {
						 if (esUsuario(i))
					 ...
					 for (int i = 1; i < 4; i++)
					 {
						 ...
						 {									
							 ...
							 ->Read(bufferLectura, 0, bufferLectura->Length);
							 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
								 || safe_cast<bool>(bufferLectura[2]) != frasePideCinco)
							 {
								 escribirMensaje("Error: no se recibe nada del jugador " +
									 jugadores[i]->getNombre());
								 pausar();
				 ...
				 return frasePideCinco;
			 }

			 bool obtenerPideTodas(int jugador)
			 {
				 resaltarJugador((jugador+0)%4, true);
				 resaltarJugador((jugador+3)%4, false);
				 bool frasePideTodas = false;
				 if (esUsuario(0)) // Alguien juega
				 {
					 if (esOnline()) // Es online
					 {
						 if (soyServidor) // Servidor
						 {
							 if (jugador == 0) // Me toca a mi
							 {
								 ...
								 pausar();
								 // Empaqueto lo que elijo y lo envío a todos los clientes conectados
								 info = gcnew array<Object^>{usuarioPideTodas};
								 bufferEscritura = empaquetarMensaje(hablarTodas, info);
								 for (int i = 1; i < 4; i++)
								 {
									 if (esUsuario(i))
									 {									
								 ...
								 for (int i = 1; i < 4; i++)
								 {
									 ...
									 {									
										 bytesLeidos = streams[jugadores[i]->getNumCliente()]->Read(bufferLectura, 0, bufferLectura->Length);
										 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
											 || safe_cast<bool>(bufferLectura[2]) != usuarioPideTodas)
										 {
											 escribirMensaje("Error: no se recibe nada del jugador " +
												 jugadores[i]->getNombre());
											 pausar();
							 ...
							 else // No me toca a mi
							 {
								 if (esUsuario(jugador)) // Es un cliente
								 {
									 ...
									 if (bytesLeidos > 0)
									 {
										 usuarioPideTodas = safe_cast<bool>(bufferLectura[2]);
										 // Le envío el ACK con su elección
										 info = gcnew array<Object^>{usuarioPideTodas};
										 bufferEscritura = empaquetarMensaje(ack, info);
										 streams[jugadores[jugador]->getNumCliente()]
				 ...
				 frasePideTodas = jugadores[jugador]->pedirTodas(mesa, usuarioPideTodas);

				 if (esUsuario(0) && esOnline() && soyServidor && jugador != 0 && !esUsuario(jugador)) 
					 // Tengo que pasarles a los demás lo que ha tirado el Ordenador
				 {
					 info = gcnew array<Object^>{frasePideTodas};
					 bufferEscritura = empaquetarMensaje(hablarTodas, info);
					 for (int i = 1; i < 4; i++)
					 {
						 if (esUsuario(i))
					 ...
					 for (int i = 1; i < 4; i++)
					 {
						 ...
						 {									
							 ...
							 ->Read(bufferLectura, 0, bufferLectura->Length);
							 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
								 || safe_cast<bool>(bufferLectura[2]) != frasePideTodas)
							 {
								 escribirMensaje("Error: no se recibe nada del jugador " +
									 jugadores[i]->getNombre());
								 pausar();
				 ...
				 return frasePideTodas;
			 }

			 int obtenerCartaElegida(int jugador, int turno)
			 {
				 resaltarJugador((jugador+0)%4, true);
				 resaltarJugador((jugador+3)%4, false);
				 int cartaElegida = 0;
					 ...
					 if (esOnline()) // Es online
					 {
						 if (soyServidor) // Servidor
						 {
							 if (jugador == 0) // Me toca a mi
							 {
								 ...
								 refrescar();
								 // Empaqueto lo que elijo y lo envío a todos los clientes conectados
								 info = gcnew array<Object^>{usuarioCartaElegida};
								 bufferEscritura = empaquetarMensaje(tirarCartaMensaje, info);
								 for (int i = 1; i < 4; i++)
								 {
									 if (esUsuario(i))
									 {									
								 ...
								 for (int i = 1; i < 4; i++)
								 {
									 ...
									 {									
										 bytesLeidos = streams[jugadores[i]->getNumCliente()]->Read(bufferLectura, 0, bufferLectura->Length);
										 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
											 || safe_cast<int>(bufferLectura[2]) != usuarioCartaElegida)
										 {
											 escribirMensaje("Error: no se recibe nada del jugador " +
												 jugadores[i]->getNombre());
											 pausar();
							 ...
							 else // No me toca a mi
							 {
								 if (esUsuario(jugador)) // Es un cliente
								 {
									 ...
									 if (bytesLeidos > 0)
									 {
										 usuarioCartaElegida = safe_cast<int>(bufferLectura[2]);
										 // Le envío el ACK con su elección
										 info = gcnew array<Object^>{usuarioCartaElegida};
										 bufferEscritura = empaquetarMensaje(ack, info);
										 streams[jugadores[jugador]->getNumCliente()]
				 ...
				 cartaElegida = jugadores[jugador]->elegirCarta(turno, mesa, usuarioCartaElegida, mazosOriginales[jugador]);

				 if (esUsuario(0) && esOnline() && soyServidor && jugador != 0 && !esUsuario(jugador)) 
					 // Tengo que pasarles a los demás lo que ha tirado el Ordenador
				 {
					 info = gcnew array<Object^>{cartaElegida};
					 bufferEscritura = empaquetarMensaje(tirarCartaMensaje, info);
					 for (int i = 1; i < 4; i++)
					 {
						 if (esUsuario(i))
					 ...
					 for (int i = 1; i < 4; i++)
					 {
						 ...
						 {									
							 ...
							 ->Read(bufferLectura, 0, bufferLectura->Length);
							 if (bytesLeidos == 0 || safe_cast<mensajesJuego>(bufferLectura[0]) != ack
								 || safe_cast<int>(bufferLectura[2]) != cartaElegida)
							 {
								 escribirMensaje("Error: no se recibe nada del jugador " +
									 jugadores[i]->getNombre());
								 pausar();
...
/// FALLO: EL ESTUCHE SE COMPRUEBA ANTES DE VER SI HAN PASADO O NO Y A VECES
/// LES PONE 2 PIEDRAS EN VEZ DE 0