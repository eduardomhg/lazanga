...
public:
	Mesa(int jugadorManoPrimero);

	Mesa(const Mesa% mesa);

	Mesa% operator =(const Mesa% mesa);
	...
	void barajar(Baraja^ baraja, int semilla);

	void repartir(Baraja^ baraja, array<Jugador^>^ jugadores);
	...
	void cambiaPorZanga(array<Jugador^>^ jugadores, int jugador);

	void recambiaSusCartas(array<Jugador^>^ jugadores, int jugador);
	...
	void comprobarEstuche(array<Jugador^>^ jugadores);
