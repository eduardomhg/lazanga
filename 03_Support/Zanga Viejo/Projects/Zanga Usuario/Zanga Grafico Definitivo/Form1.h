...
#pragma endregion
	...
	private:
		...
		const static bool usuarioJuega = true;
		...
		void mainHiloJuego()
		{
			...
			jugadores[1] = Jugador("Antonio", 1, 4);
			jugadores[2] = Jugador("Cris", 2, 4);
			jugadores[3] = Jugador("Crisanto", 3, 4);
			...
			if (usuarioJuega && !(mesa.getAlguienSolo() && mesa.getJugadorSolo() == 2))
			{
				...
				actualizarCartasJugador(jugadores, 0);
			}
...
private: System::Void botonComenzar_KeyPress(System::Object^  sender, System::Windows::Forms::KeyPressEventArgs^  e) {
			 if (e->KeyChar == 'x')
				 {
					 if (hiloSuspendido == false)
						 pausar();
